...
export class PhotoService {
  ...
  async create(file: Express.Multer.File, data: CreatePhotoBodyDto) {
  ...
  async findAll(itemId: number, type: PhotoType) {
    return await this.prismaService.photo.findMany({
      where: {
        OR: [
          {
            AND: [{ contentItemId: itemId }, { type }],
          },
          {
            AND: [{ userId: itemId }, { type }],
          },
        ],
      },
    });
  ...
  async findOne(itemId: number, type: PhotoType) {
    const data = await this.prismaService.photo.findUnique({
