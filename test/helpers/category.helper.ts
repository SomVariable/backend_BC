...
import { Area, CategoryTranslationType, Practice, Service } from '@prisma/client';
...
export const createArea = async (app, { jwtToken }: FirstUserOkResponse) => {
    ...
    const responseBody: CreatedAreaOkResponse = await JSON.parse(createAreaRes.text);
...
export const updateArea = async (
    ...
    const responseBody: UpdatedAreaOkResponse = await JSON.parse(createAreaRes.text);
    ...
    const containsValues = [
        ...responseBody.data.practicesIds
    ]
...
export const updateAreaAddDelFunction = async (
    ...
    const responseBodyAdd: UpdatedAreaOkResponse = await JSON.parse(updateAreaResWithAdded.text);
    ...
    expect([...responseBodyAdd.data.practicesIds].map(practice => practice.id)).toEqual(
    ...
    const responseBody: UpdatedAreaOkResponse = await JSON.parse(areaResAfterDeleted.text);
...
export const getAreaFull = async (app, area: Area) => {
    ...
    const responseBody: GetAreaFullOkResponse = await JSON.parse(createAreaRes.text);
...
export const createCategoryInfo = async (
    ...
    const responseBody: CategoryTranslationCreatedOkResponse = await JSON.parse(createAreaRes.text);

    expect(responseBody).toHaveProperty('message');
    expect(responseBody).toHaveProperty('data');
    expect(responseBody.data).toHaveProperty('id');
    expect(responseBody.data).toHaveProperty('categoryTranslationType', dto.categoryTranslationType);
...
export const updateCategoryInfo = async (
    ...
    const responseBody: CategoryTranslationUpdatedOkResponse = await JSON.parse(createAreaRes.text);
...
export const deleteArea = async (app, { jwtToken }: FirstUserOkResponse, area: Area) => {
    const createAreaRes = await request(app.getHttpServer())
        .delete(`/area/${area.id}`)
        .set('User-Agent', 'Mobile')
        .set('Authorization', `Bearer ${jwtToken}`)
    ...
    const responseBody: DeletedAreaOkResponse = await JSON.parse(createAreaRes.text);
    expect(responseBody).toHaveProperty('message');
    expect(responseBody).toHaveProperty('data');
    expect(responseBody.data).toHaveProperty('id');
...
export const createService = async (app, { jwtToken }: FirstUserOkResponse) => {
    ...
    const responseBody: CreatedOfferOkResponse = await JSON.parse(createAreaRes.text);
    expect(responseBody).toHaveProperty('message');
    expect(responseBody).toHaveProperty('data');
    expect(responseBody.data).toHaveProperty('id');
...
export const updateService = async (app, { jwtToken }: FirstUserOkResponse, service: Service, practiceIds: number[] = []) => {
    ...
    const responseBody: UpdatedOfferOkResponse = await JSON.parse(createAreaRes.text);
    ...
    expect(practiceIds).toEqual(
        expect.arrayContaining([...responseBody.data.practicesIds].map(category => category.id)
        ));
...
export const deleteService = async (app, { jwtToken }: FirstUserOkResponse, service: Service) => {
    const createAreaRes = await request(app.getHttpServer())
    ...
    const responseBody: DeletedOfferOkResponse = await JSON.parse(createAreaRes.text);
    expect(responseBody).toHaveProperty('message');
    expect(responseBody).toHaveProperty('data');
    expect(responseBody.data).toHaveProperty('id');
...
export const createPractice = async (app, { jwtToken }: FirstUserOkResponse) => {
    ...
    const responseBody: CreatedPracticeOkResponse = await JSON.parse(createPracticeRes.text);
    expect(responseBody).toHaveProperty('message');
    expect(responseBody).toHaveProperty('data');
    expect(responseBody.data).toHaveProperty('id');
...
export const updatePractice = async (
    ...
    const responseBody: UpdatedPracticeOkResponse = await JSON.parse(updatePracticeRes.text);
    ...
    expect(areasIds).toEqual(
        expect.arrayContaining([...responseBody.data.areasIds].map(area => area.id)
        ));
    expect(servicesIds).toEqual(
        expect.arrayContaining([...responseBody.data.servicesIds].map(service => service.id)
        ));
    return responseBody
}
...
export const getPractice = async (app, service: Service) => {
    ...
    const responseBody: GetPracticeOkResponse = await JSON.parse(createPracticeRes.text);
    expect(responseBody).toHaveProperty('message');
    expect(responseBody).toHaveProperty('data');
    expect(responseBody.data).toHaveProperty('id');
...
export const deletePractice = async (app, { jwtToken }: FirstUserOkResponse, practice: Practice) => {
    const createPracticeRes = await request(app.getHttpServer())
    ...
    const responseBody: DeletedPracticeOkResponse = await JSON.parse(createPracticeRes.text);
    expect(responseBody).toHaveProperty('message');
    expect(responseBody).toHaveProperty('data');
    expect(responseBody.data).toHaveProperty('id');
...
export const areaCRUD = async (app, data, _, { responseBody }: signUpAdminType) => {
    ...
    const idArray = [practice_2Res.data.id, practice_3Res.data.id, practice_1Res.data.id]
    await updateArea(app, responseBody, createAreaRes.data, idArray)
    //second-update
    ...
    const idArray_2 = [practice_4Res.data.id, practice_5Res.data.id, practice_6Res.data.id]

    await updateAreaAddDelFunction(app, responseBody, createAreaRes.data, idArray_2)
...
export const serviceCRUD = async (app, data, _, { responseBody }: signUpAdminType) => {
...
export const practiceCRUD = async (app, data, _, { responseBody }: signUpAdminType) => {
    ...
    const idServiceArray = [service_2Res.data.id, service_3Res.data.id, service_1Res.data.id]
    await updatePractice(app, responseBody, createPracticeRes.data, idAreaArray, idServiceArray)
    await getPractice(app, createPracticeRes.data)
    await createCategoryInfo(app, responseBody, createPracticeRes.data, "PRACTICE")
    await updateCategoryInfo(app, responseBody, createPracticeRes.data, "PRACTICE")
