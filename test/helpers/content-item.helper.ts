...
export const createContentItem = async (
  ...
  const responseBody: CreatedContentItemOkResponse = await JSON.parse(
    response.text,
  );
...
export const getContentItem = async (
  ...
  const responseBody: GetContentItemOkResponse = await JSON.parse(
    response.text,
  );
...
export const createContentItemInfo = async (
  ...
  const _responseBody: CreatedContentItemInfoOkResponse = await JSON.parse(
    response.text,
  );
...
export const updateContentItemInfo = async (
  ...
  const responseBody: UpdatedContentItemInfoOkResponse = await JSON.parse(
    response.text,
  );
...
export const contentItemF = async (app, _, mockUser, data: signUpAdminType) => {
  const createContentItemRes = await createContentItem(
    app,
  ...
  try {
    ...
    await createContentItemInfo(
      app,
      data.responseBody.jwtToken,
      createContentItemRes.data,
    );
    await updateContentItemInfo(
      app,
      data.responseBody.jwtToken,
      createContentItemRes.data,
    );
...
export const createTag = async (app, jwt: string, tagDTO: CreateTagDto) => {
  const response = await request(app.getHttpServer())
    .post(`/tag`)
...
export const getTag = async (app, jwt: string, tag: Tag) => {
  const response = await request(app.getHttpServer())
    .get(`/tag/${tag.id}`)
    .set('Authorization', `Bearer ${jwt}`)
    .set('User-Agent', 'Mobile')
...
export const deleteTag = async (app, jwt: string, tag: Tag) => {
  const response = await request(app.getHttpServer())
    .delete(`/tag/${tag.id}`)
    .set('Authorization', `Bearer ${jwt}`)
    .set('User-Agent', 'Mobile')
...
export const createTagInfo = async (app, jwt: string, tag: Tag) => {
...
export const updateTagInfo = async (app, jwt: string, tag: Tag) => {
  const tagDTO: CreateTagInfoDto = {
    tag: 'Tag@#$!$',
  };
  const response = await request(app.getHttpServer())
...
export const tagPracticeCI_F = async (
  app,
  _: null,
  mockUser,
  adminData: signUpAdminType,
  ...
  const contentItem_1 = await createContentItem(
    app,
    adminData.responseBody.jwtToken,
  );
  const contentItem_2 = await createContentItem(
    app,
    adminData.responseBody.jwtToken,
  );
  const contentItem_3 = await createContentItem(
    app,
    adminData.responseBody.jwtToken,
  );
  const contentItem_4 = await createContentItem(
    app,
    adminData.responseBody.jwtToken,
  );
