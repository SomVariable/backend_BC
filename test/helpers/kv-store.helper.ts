...
export const getSession = async (app: INestApplication, userId: number, session? : Partial<Session>) => {
...
export const activeSession = async (app, id: number) => {
    const responseKvStoreActive = await request(app.getHttpServer())
      ...
      const responseKvStoreActiveBody = await JSON.parse(responseKvStoreActive.text)
